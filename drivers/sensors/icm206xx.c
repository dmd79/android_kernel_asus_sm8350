 ...
 */
static int icm_i2c_read_reg(struct icm_sensor *sensor, u8 reg, u8 *data, int len)
{
	int res;
	uint8_t subaddr[1];
	...
	res = i2c_transfer(sensor->i2c_client->adapter, msg, ARRAY_SIZE(msg));
	mutex_unlock(&sensor->bus_lock);
	if (res != ARRAY_SIZE(msg)) {
		icm_errmsg("Failed: %d != %zu\n", res, ARRAY_SIZE(msg));
		return -1;
	} else{
		return 0;
	}
}
static int icm_spi_read_reg(struct icm_sensor *sensor, u8 reg, u8 *data, int len)
{
...
{
	if (sensor->pdev) {
		return icm_spi_read_reg(sensor, reg, data, len);
	} else if (sensor->i2c_client) {
		return icm_i2c_read_reg(sensor, reg, data, len);
	} else{
		icm_errmsg("NULL bus!\n");
		return -1;
	}
...
static s32 icm_read_byte_data(struct icm_sensor *sensor, u8 reg)
{
	...
	int res;
	if (sensor->pdev) {
		res = icm_spi_read_reg(sensor, reg, &data, 1);
	} else if (sensor->i2c_client) {
		res = icm_i2c_read_reg(sensor, reg, &data, 1);
	} else{
		icm_errmsg("NULL bus!\n");
		res = -1;
	}
...
static s32 icm_i2c_do_write_byte_data(struct icm_sensor *sensor, u8 reg, u8 data)
{
	int res;
	uint8_t d[2];
	struct i2c_msg msg[] = {
...
static s32 icm_spi_do_write_byte_data(struct icm_sensor *sensor, u8 reg, u8 data)
{
	struct spi_message msg;
	int res;
	u8 d[2];
...
{
	if (sensor->pdev) {
		return icm_spi_do_write_byte_data(sensor, reg, data);
	} else if (sensor->i2c_client) {
		return icm_i2c_do_write_byte_data(sensor, reg, data);
	} else{
		icm_errmsg("NULL bus!\n");
		return -1;
	}
...
static int icm206xx_init_regulator(struct icm_sensor *sensor)
{
	...
	if (IS_ERR(sensor->power_supply)) {
		...
		return -EINVAL;
	} else if (regulator_count_voltages(sensor->power_supply) > 0) {
		if (regulator_set_voltage(sensor->power_supply, 1710000, 3450000)) {
			icm_errmsg("Failed to set voltage to power supply\n");
		}
	}
	...
	} else if (regulator_count_voltages(sensor->vdd) > 0) {
		if (regulator_set_voltage(sensor->vdd, 1710000, 3450000)) {
			icm_errmsg("Failed to set voltage to vdd\n");
		}
	}
	return 0;
}
 ...
 */
static int icm_probe(struct i2c_client *a_client,
	struct spi_device *a_pdev, struct device *a_dev)
{
	struct icm_sensor *sensor;
	struct icm_spi_data *pdata;
	...
	if (sensor->dev->of_node) {
		ret = icm_parse_dt(sensor->dev, pdata);
...
static int icm_i2c_probe(struct i2c_client *a_client,
	const struct i2c_device_id *a_id)
{
	if (!icm_i2c_check_bus(a_client)) {
		icm_dbgmsg("invalid bus! might need to modify trustzone.");
		return -1;
	}
	return icm_probe(a_client, NULL, &a_client->dev);
}
 ...
 */
static int icm_i2c_remove(struct i2c_client *client)
{
	icm_dbgmsg("+\n");
	icm_deinit();
	icm_dbgmsg("-\n");
...
static const struct i2c_device_id icm_i2c_id[] = {
	{ICM_SENSOR_NAME, 0},
	{}
};
...
static const struct of_device_id icm_i2c_of_match[] = {
	{ .compatible = "invn,icm20690-i2c", },
	{ },
};
MODULE_DEVICE_TABLE(of, icm_of_match);
MODULE_DEVICE_TABLE(of, icm_i2c_of_match);
...
static struct i2c_driver icm_i2c_driver = {
	.id_table = icm_i2c_id,
	.probe          = icm_i2c_probe,
	.remove         = icm_i2c_remove,
	.driver = {
		.name   = ICM_SENSOR_NAME,
		.owner  = THIS_MODULE,
		.pm     = &icm_pm,
		.of_match_table = icm_i2c_of_match,
	},
};

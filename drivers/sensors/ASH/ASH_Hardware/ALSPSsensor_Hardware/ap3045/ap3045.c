...
#undef dbg
	...
	#define dbg(fmt, args...)
#endif
#define log(fmt, args...) pr_debug("[%s][%s]"fmt,MODULE_NAME,SENSOR_TYPE_NAME,##args)
...
static int ap3045_proximity_hw_set_config(void)
{
	...
	if (ret < 0)		
		return ret;
/*
	ret = ap3045_proximity_hw_set_mean(AP3045_PS_MEAN_2);
	if (ret < 0)		
		return ret;

	ret = ap3045_proximity_hw_set_integration(AP3045_PS_INTEG_5);
	if (ret < 0)		
		return ret;

	ret = ap3045_proximity_hw_set_persistence(AP3045_PS_PERSIS_1);
	if (ret < 0)		
		return ret;
*/
	return 0;
...
static int ap3045_light_hw_set_config(void)
{
	...
	ret = ap3045_light_hw_set_integration(AP3045_ALS_GAIN_8576);
	if (ret < 0)		
		return ret;
	//+++
	if (g_ASUS_hwID <= ZE500KL_ER1){
		ret = ap3045_light_hw_set_integration(ap3045_ALS_GAIN_34304);
		if (ret < 0)		
			return ret;
	}else{
		ret = ap3045_light_hw_set_integration(ap3045_ALS_GAIN_2144);
		if (ret < 0)		
			return ret;
	}
	//---
...
static int ap3045_IRsensor_hw_set_interrupt(void)
{
	...
	data_buf = i2c_read_reg_u8(g_i2c_client, AP3045_REG_SYS_INTCTRL);
	if (data_buf < 0) {
		err("ap3045_IRsensor_hw_set_interrupt read ERROR\n");
		return data_buf;
	}
	dbg("ap3045_IRsensor_hw_set_interrupt read (0x%02X) \n", data_buf);

	data_origin = data_buf;
	data_buf |= AP3045_SYS_ICLEAN_MANUAL;

	ret = i2c_write_reg_u8(g_i2c_client, AP3045_REG_SYS_INTCTRL, data_buf);
	if (ret < 0) {
		err("ap3045_IRsensor_hw_set_interrupt write ERROR \n");
		return ret;
	} else {
		dbg("ap3045_IRsensor_hw_set_interrupt write (0x%X -> 0x%X) \n", data_origin, data_buf);
	}
